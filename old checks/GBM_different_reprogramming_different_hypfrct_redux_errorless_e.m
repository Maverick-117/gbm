...
Doses = [2 10]; % dose fraction sizes in Gy
Frac = [30 6]; 
len_D = length(Doses);
...
n = 1; c_l = 4;%6;
C = [0 5.196*10.^(linspace(-5,-2,c_l))];% 1]; 
color = cell(1,length(C));
for jj=1:c_l
    color{jj+1} = [jj/c_l (c_l-jj)/c_l 1];% cyan to magenta
end
color{1} = [0 0 0]
...
for jj=1:len_D
    ...
    for g = 1:length(cell_lines)
        ...
        for k = 1:length(C)
            ...
            figure(1)
            hold on
            plot(Tb(:,1),log(Ub(:,1)*total_cell_num),'g','LineStyle','--','LineWidth', line_lengths(jj)) % stem cell
            plot(Tb(:,1),log(Ub(:,2)*total_cell_num),'g','LineWidth', line_lengths(jj)) 
...
title(['\fontsize{14} ',fdbk_type,' Fdbk, Dose:',num2str(Doses(2)),' Gy, Days:',num2str(Frac(2))])
%fig2 = figure(10);
