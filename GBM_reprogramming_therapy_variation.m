...
n = 1; low = -5; high = -2; c_l = 1*(high-low+1);
...
len_surv_cont = length(cont_p_b);
dose_reprog_time_cont = zeros(len_D,len_C,len_samples,len_surv_cont);

for ii=1:len_surv_cont
    surv_vec = {cont_p_a, cont_p_b(ii), compt_mult, srvn_csc, srvn_dcc}; %assuming these control parameters are constantd
    for jj=1:len_D
        ...
        for g = 1:length(cell_lines)
            ...
            for k = 1:length(C)
                ...
                if jj > 1
                    dose_reprog_time_cont(jj,k,:,ii) = U(sample_times,1);
                else
                    dose_reprog_time_cont(jj,k,:,ii) = U(sample_times+dose1offset,1);
                end
    %            x = [];
    %             y1 = [];
    %             y2 = [];
...
fig_start = 50;
for ii=1:len_surv_cont
    title_prefix = [fdbk_type,' Fdbk; Survivin Control Params:(' num2str(cont_p_a) ',' num2str(cont_p_b(ii)) ',' num2str(compt_mult) '); Relative CSC at time ~ '];
    for t=1:len_samples
        figX = figure(50*ii+(t-1));
        hold on;
        for k=1:len_C
            plot(Doses,dose_reprog_time_cont(:,k,t,ii)/dose_reprog_time_cont(2,k,t,ii),'o',...
                'color', color{k},'LineStyle',line_types{k},'LineWidth',line_lengths(2));
            per_ratio(:,k,t,ii) = max(0,1-dose_reprog_time_cont(:,k,t,ii)/dose_reprog_time_cont(2,k,t,ii));
        end
...
% fig_start2 = 60;
% control_ratio = cont_p_b/cont_p_a;
% for t=1:len_samples
